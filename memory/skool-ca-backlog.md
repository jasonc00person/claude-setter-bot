# Skool CA Outreach Backlog

> Members not yet messaged. Work through during Skool workflow sessions.
> Active/in-progress members are tracked in `pipeline-state.md`.

## Opener Sent -- No Reply (Mar 3-5)

> Already contacted. Do NOT double text. Only re-check if they resurface.

Yahia Farid (Dubai), Aleksa Samardzic (LA), Giacomo Bruneri (Rome), Maria De la Cuesta (Madrid), Kier Porter (London), Cyrus Lancini (Brisbane), Dylan Vandamme (Brussels), Claudia-Rabia Monnig (Berlin), Jarrett Campbell (LA, cancelling), Graham Radina (Chicago, cancelled), Robert Horvath (Stockholm), Alexander Darras (Munich, cancelled)

## Already Messaged -- No Reply (Mar 3-5 batch)

> These were all contacted Mar 5 during Sessions 3-7. Confirmed via chat history. Do NOT double text.

Yohan Thomas (Edmonton), James Wu (Taiwan — geo fail), Josh Hernandez (Denver), Simon Liu (SF), Niklas Falkenstein (Berlin), Armaan Sidhu (LA), Vincent Huisman (Amsterdam), Justin Klein (NY), Hakeem Hoang (Melbourne), Brett Fullerton (Phoenix), Kelvin Hsu (NY), Otavio Zerbini (Miami), Mike Oddo (Phoenix, Real Estate)

## Not Yet Messaged / Needs Verification (Target Geo)

> Many of these may also have been messaged Mar 5. Check chat before sending.

| Member | Location | Tier |
|-|-|-|
| Amir Tai | Chicago (cancelled) | -- |
| Lawrence Johnson | Chicago (cancelling) | -- |
| ~~Laraib Lodhi~~ | ~~London~~ | ~~Moved to pipeline Stage 4~~ |
| Jano S. | Berlin (cancelling) | -- |
| Leroy Moritz | Dubai (declining) | -- |
| Maciej Ca | Warsaw (declining) | -- |
| Ricky A | Chicago (inactive 40d) | -- |
| Alexandre Buisson | Dubai | -- |
| Media Flooding | NY | -- |
| Steve Young | Denver | -- |
| Spencer Simonson | Las Vegas | -- |
| Azan Durman | NY | -- |
| Tristan Martini | Chicago | -- |
| Emilio Antun | Chicago | -- |
| Kacper Krasinski | Warsaw | -- |
| Jocelyn Sisko | NY | -- |
| Yvonne Briones | Barcelona | -- |
| Mustafa Kurtulus | Istanbul | -- |
| Briar Cochran | Chicago | -- |
| Rob Scan | Edmonton | -- |
| Richard Boateng | London | -- |
| Filip Simeunovic | Budapest | -- |
| Ana Carolina Pereira | NY (cancelling) | -- |
| Leonid Kim | LA | -- |
| Pedro Branco | London | -- |
| ~~Kelvin Hsu~~ | ~~NY~~ | ~~Messaged Mar 5, no reply~~ |
| Larry Fan | California | Surgeon, not ICP. Skip |
| David W | Sydney | -- |
| Nick Stacy | -- | -- |
| Jazmina Lopez | Mexico City | -- |
| Edgar Pfohl | Paris | -- |
| Abdel Kader Jrab | Dubai | -- |
| ~~Otavio Zerbini~~ | ~~Miami~~ | ~~Messaged Mar 5, no reply~~ |
| ~~Justin Klein~~ | ~~NY~~ | ~~Messaged Mar 5, no reply~~ |
| ~~Brett Fullerton~~ | ~~Phoenix~~ | ~~Messaged Mar 5, no reply. Declined/churning~~ |
| Gabe Saporta | LA | -- |
| Marti Fox | Dubai | -- |
| ~~Vincent Huisman~~ | ~~Amsterdam~~ | ~~Messaged Mar 5, no reply~~ |
| John Ladd | Berlin | -- |
| ~~Hakeem Hoang~~ | ~~Melbourne~~ | ~~Messaged Mar 5, no reply~~ |
| Shayan Maredia | Austin TX | Free member, referred by Jason. Skip |
| Mikhail Nadeson | -- | $75/mo |
| Elijah Bricker | NY | $800/yr (cancelled) |
| Tulio Sasaya | LA | $80/mo |
| Samuel Millsap | Chicago | Free |
| Santiago Castro | LA | $80/mo (active 44d ago) |
| ~~Mike Oddo~~ | ~~Phoenix~~ | ~~Messaged Mar 5, no reply. $480/yr~~ |

## Moved to Pipeline (Active)

- Mateja Proucil (Osijek) -- Stage 3, permission sent Mar 7
- Jaylen Rodriguez (NYC) -- Stage 2, AV sent Mar 7
- Christian Kadivec (Ljubljana) -- Stage 1, opener sent Mar 7
- Jazmin Bautista (LA) -- Stage 1, opener sent Mar 7
- Liam Larsson (Stockholm) -- Stage 3, re-engagement, permission sent Mar 7
- Koby Luedtke (Chicago) -- Stage 4, Q1 sent Mar 7
- Collin Bexten (NY) -- Stage 3, permission sent Mar 7
- Austin Coldiron (Bali) -- Stage 4, Q1 sent
- Kaden Green (LA) -- Stage 4, Q3 sent
- Dane J (Chicago) -- Stage 4, Q3 sent
- Adam Ziak (Iceland) -- Stage 6, call booked Mar 9
- Kunal Jiwani -- Stage 6, call booked Mar 9
- Irfan Aziz (London) -- Stage 5, bridge sent Mar 6
- Aaron Harun Guner (KL) -- Stage 6, call booked Mar 10
- Pat Divilly (Ireland) -- Stage 6, Calendly sent
- Cristiano Capasso (Rome) -- Stage 6, call booked Mar 9
- Tyler Panzner (NY) -- FLAGGED, requesting refund
- Allie Thaxton (NY) -- Opener sent Mar 6
- Goncalo Goncalves (Lisbon) -- Call booked pre-session
- Victoria Denisse (Melbourne) -- Call completed Mar 3-4
- Jose Avila (LA) -- Stage 1, welcome back opener sent Mar 8
- Rayyan Rehman (Karachi) -- DISQUALIFIED, geo filter (Pakistan)
- Nuno Machado Lopes (Lisbon) -- Stage 1, opener sent Mar 13. DNA Health & Fitness Coach
- Equipo Expansión (Buenos Aires) -- SKIPPED, geo filter (Argentina/South America)
- Tyler Panzner (NY) -- Stage 6, Calendly sent Mar 13. Re-engaged, agreed to call

*~30 unmessaged (many confirmed messaged Mar 5), ~20 opener sent no reply, ~25 active in pipeline. Last updated: March 13, 2026*
